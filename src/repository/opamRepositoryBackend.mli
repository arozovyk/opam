...
type update =
  ...
  (** A patch file and the list of files it changes.
      ...
      the upstream state. The second element lists the files
      affected by the patch as absolute paths. *)
  | Update_empty
...
(** [get_diff parent_dir subdir1 subdir2] computes the diff between the two
    subdirs of [parent_dir], returns None if they are equal, and a patch file
      with list of changed files otherwise. The changed files list contains
