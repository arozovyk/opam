...
let apply_repo_update repo repo_root = function
  ...
  | Update_patch (f, changed_files) ->
    ...
    OpamStd.Option.map_default raise (Done changed_files) err
  | Update_empty ->
...
let update repo repo_root =
  ...
  | (Update_full _ | Update_patch _) as upd ->
    ...
    | true ->
      apply_repo_update repo repo_root upd @@+ fun changed_files ->
      B.repo_update_complete repo_root repo.repo_url @@+ fun () ->
      Done (`Changes changed_files)
