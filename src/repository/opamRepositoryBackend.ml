...
let get_diff_tar_dir tar_file dir =
  ...
  let rec read_dir_contents acc prefix current_dir =
    ...
    List.fold_left (fun acc entry ->
        ...
        | Unix.S_DIR ->
          read_dir_contents acc relative_path (OpamFilename.Dir.of_string full_path)
  ...
  in
  let dir_contents = read_dir_contents OpamStd.String.Map.empty "" dir in
